...
public class LeilaoTest {
	...
	@Test
	public void receberUmLanceComMatcher() {
		Leilao leilao = new CriadorDeLeilao().para(MCBOOKPRO15).construir();
		assertEquals(0, leilao.getLances().size());
	...
	public void receberVariosLances() {
		...
		Leilao leilao = new CriadorDeLeilao().para(MCBOOKPRO15)
				.lance(steveJobs, 2000).lance(steveWozniak, 3000).construir();
	...
	public void naoAceitarDoisLancesSeguidosDoMesmoUsuario() {
		...
		Leilao leilao = new CriadorDeLeilao().para(MCBOOKPRO15)
				.lance(steveJobs, 2000).lance(steveJobs, 2000).construir();
	...
	public void naoAceitarMaisDoQueCincoLancesDeUmMesmoUsuario() {
		...
		Leilao leilao = new CriadorDeLeilao().para(MCBOOKPRO15)
				.lance(steveJobs, 2000.0).lance(billGates, 3000.0)
				.lance(steveJobs, 4000.0).lance(billGates, 5000.0)
				.lance(steveJobs, 6000.0).lance(billGates, 7000.0)
				.lance(steveJobs, 8000.0).lance(billGates, 9000.0)
				.lance(steveJobs, 10000.0).lance(billGates, 11000.0)
				// Deve ser ignorado
				.lance(steveJobs, 12000.0).construir();
	...
	public void dobrarOuUltimoLanceDado() {
		Leilao leilao = new CriadorDeLeilao().para(MCBOOKPRO15)
				.lance(steveJobs, 2000).lance(billGates, 3000).construir();
